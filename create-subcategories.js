...
async function waitForBackend() {
  console.log('⏳ Vérification que le backend local est disponible...');
  const maxRetries = 5;
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      await axios.get(`http://localhost:4000/health`, { timeout: 5000 });
      console.log('✅ Backend local disponible!\n');
      return true;
    } catch (error) {
      if (i < maxRetries - 1) {
        console.log(`   ⏳ Backend pas encore prêt, réessai dans 2 secondes...`);
        await new Promise(resolve => setTimeout(resolve, retryDelay));
      }
    }
  }
  throw new Error('Backend local toujours indisponible après 10 secondes');
}
    ...
    for (const parentCategory of existingCategories) {
      ...
      for (const subcat of subcategoriesConfig) {
        try {
          ...
          const response = await api.post('/products/categories', subcategoryData);
          createdSubcategories.push(response.data.category);
          ...
          if (error.response?.data?.message?.includes('existe déjà')) {
            ...
            console.log(`   ❌ Erreur lors de la création de ${subcat.name}:`, error.response?.data?.message || error.message);
          }
        }
      }
      ...
      if (categoryProducts.length > 0 && createdSubcategories.length > 0) {
        ...
        for (const product of categoryProducts) {
          try {
            ...
            console.log(`   ❌ Erreur pour ${product.name}:`, error.response?.data?.message || error.message);
          }
        }
      }
    }
