...
const SUBCATEGORIES_CONFIG: Record<string, Array<{name: string, slug: string}>> = {
...
function determineSubcategory(product: any, parentCategory: any, subcategories: any[]) {
  ...
  for (const [pattern, subcatName] of Object.entries(keywordMap)) {
    ...
    if (regex.test(text)) {
      const subcat = subcategories.find((s: any) => s.name === subcatName);
...
async function main() {
  try {
    console.log('🚀 Création des sous-catégories en PRODUCTION...\n');
    ...
    for (const parentCategory of existingCategories) {
      ...
      console.log(`   Nombre de produits: ${(parentCategory._count as any)?.products || 0}`);
      ...
      const categoryProducts = products.filter((p: any) => p.categoryId === parentCategory.id);
