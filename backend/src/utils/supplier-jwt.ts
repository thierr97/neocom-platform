...
export const verifySupplierAccessToken = (token: string): SupplierTokenPayload => {
  return jwt.verify(token, (process.env.SUPPLIER_JWT_SECRET || process.env.JWT_SECRET)!) as SupplierTokenPayload;
};

export const verifySupplierRefreshToken = (token: string): SupplierTokenPayload => {
  return jwt.verify(token, (process.env.SUPPLIER_JWT_REFRESH_SECRET || process.env.JWT_REFRESH_SECRET)!) as SupplierTokenPayload;
};
