/**
 * Écran de sélection de client pour création de facture/devis
 * Étape 1 du workflow de création
 */
...
const CustomerSelectionScreen: React.FC<Props> = ({ navigation, route }) => {
  const { documentType = 'invoice' } = route.params || {};
  ...
  const loadCustomers = async () => {
    try {
      ...
      if (error.response?.status !== 403 && error.response?.status !== 404) {
        Alert.alert('Erreur', error.message || 'Erreur de chargement des clients');
      }
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const handleCustomerSelect = (customer: Customer) => {
    // Navigate to product selection with the selected customer
  ...
  const handleCreateNewCustomer = () => {
    // Navigate to CreateCustomer screen
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={styles.subtitle}>Sélectionner un client</Text>
        </View>
